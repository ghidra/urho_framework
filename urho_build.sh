	...
	if [ -z "$2" ];then
		...
		if [ -d $URHOPATH ];then
			...
			if [ -z "$3" ];then
				...
				bash $URHOPATH/cmake_generic.sh $URHOBUILD -DURHO3D_64BIT=1 -DURHO3D_OPENGL=1 -DURHO3D_LIB_TYPE=SHARED -DURHO3D_SAMPLES=1 -DURHO3D_DEPLOYMENT_TARGET=core-avx-i
			fi
			cd $URHOBUILD
			make
		else
