...
if [ $# -eq 0 ];then
	...
	if [ -z "$2" ];then
		...
		if [ -d $URHOPATH ];then
			...
			if [ -z "$3" ];then
				echo "standard build"
				bash $URHOPATH/script/cmake_generic.sh $URHOBUILD -DURHO3D_64BIT=1 -DURHO3D_OPENGL=1 -DURHO3D_LIB_TYPE=SHARED -DURHO3D_DEBUG=1 -DURHO3D_SAMPLES=1 -DVIDEO_MIR=0 
			else
				echo "build with core-avx-i"
				bash $URHOPATH/script/cmake_generic.sh $URHOBUILD -DURHO3D_64BIT=1 -DURHO3D_OPENGL=1 -DURHO3D_LIB_TYPE=SHARED -DURHO3D_SAMPLES=1 -DURHO3D_DEPLOYMENT_TARGET=generic -DURHO3D_SSE=1
			fi
			cd $URHOBUILD
			make
		else
